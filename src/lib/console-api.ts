...
// The Tune/Draft loops run the model via the Max-subscription `claude` CLI
// under the engine's launchd context (keychain auth works there; an
// API key is never used). The engine streams the output into an in-memory
// job; we start a job and then poll it so the browser can watch the draft /
// rewrite appear and every Vercel function call stays short. See the engine
// router at tasks/m1-desk-endpoint.py (deployed to ~/automation/api on M1).

export interface DeskJobStart {
  job_id: string;
  kind: string;
  model: string;
}

export interface DeskJobPoll {
  job_id: string;
  kind: string;
...
/** Start a streamed claude run on the engine. Returns a job_id to poll.
 *  The POST itself returns immediately (the model runs in a background
 *  thread on M1), so a short timeout is fine. */
export function deskStartJob(
  prompt: string,
  model: string,
  kind: string,
): Promise<ApiResult<DeskJobStart>> {
  return _post<DeskJobStart>("/desk/run", { prompt, model, kind }, { timeoutMs: 12_000 });
}

/** Poll a streamed run. While running, `text` grows with the model output;
 *  on `done` it is the final text. 404 (ApiResult http/404) means the job
 *  expired or the engine restarted mid-run. */
export function deskPollJob(jobId: string): Promise<ApiResult<DeskJobPoll>> {
  return call<DeskJobPoll>(`/desk/run/${encodeURIComponent(jobId)}`, { timeoutMs: 12_000 });
}

/** Pre-warm the engine's claude runtime so the next real Tune/Draft is the
 *  fast warm path. Fire-and-forget from the page; the engine also warms
 *  itself every few minutes via com.jont.desk-warm. */
export function deskWarm(): Promise<ApiResult<{ ok: boolean; elapsed?: number; cold?: boolean }>> {
  return _post<{ ok: boolean; elapsed?: number; cold?: boolean }>("/desk/warm", {}, { timeoutMs: 95_000 });
}
